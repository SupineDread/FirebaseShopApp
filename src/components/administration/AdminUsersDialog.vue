<template>
  <v-dialog v-model="usersDialog" max-width="500px">
    ...
    <v-card>
      ...
      <v-card-actions>
        <v-spacer/>
        <v-btn color="indigo darken-1" flat @click="close">{{ $t('common.close') }}</v-btn>
        <v-btn v-if="userDialogEditMode" color="indigo darken-1" flat @click="update">{{ $t('common.update') }}</v-btn>
        <v-btn v-else color="indigo darken-1" flat @click="add">{{ $t('common.save') }}</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
