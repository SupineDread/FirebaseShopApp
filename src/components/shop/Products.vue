<template>
  <div>
      ...
      <v-flex xs12 m6>
        <v-toolbar color="indigo" dark>
          ...
          <v-menu origin="center center" transition="scale-transition" bottom>
            <v-btn color="accent" dark slot="activator">
        ...
        <v-card>
          <v-container fluid grid-list-md>
            <v-layout row wrap>
              <v-flex :class="`xs${product.flex}`" v-for="product in products" :key="product.name">
    ...
    <v-layout row>
      ...
      <v-progress-linear v-if="loading" color="accent" indeterminate/>
    </v-layout>
    <product-sheet :product="productSheet" @closeSheet="closeSheet" :sheet="statusSheet"></product-sheet>
  </div>
</template>
