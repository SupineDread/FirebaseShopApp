...
export default {
  ...
  actions: {
    fetchAllProductsPaginate({commit, state}){
      if (!state.finish){
        db.collection('products').limit(state.limit).where('createdAt', '>', state.last).orderBy('createdAt', 'asc').onSnapshot(snapshot => {
          commit('setProducts', snapshot)
        }, error => {
          console.log('Listener products pagination is off...')
        })
      }
    }
  },
